...
#%%
np.save(results_path + 'results_centers.npy', results)

# plane = 24
# centers = results[plane][0]['centers']
# subdata = data[plane]
# print(f"found {len(centers)} blobs")
# pl.figure(figsize=(5, 5))
# # pl.matshow(LoG, cmap='gray', aspect = 0.5)
# pl.plot(centers[:, 1], centers[:, 0], 'ro', alpha=0.8, ms=0.5)
# pl.matshow(subdata, cmap='gray', aspect = 0.5)
# pl.plot(centers[:, 1]+current_crop.shape[1]/2, centers[:, 0]+current_crop.shape[0]/2, 'ro', alpha=1, ms=0.2)
# pl.xlim([0, LoG.shape[1]])
# pl.ylim([0, LoG.shape[0]])
...
# pl.plot(subpoints[row,:,1],subpoints[row,:,0],'o-',alpha=1)
# pl.figure(figsize=(5,2))
# pl.plot(subpoints[:,column,1],subpoints[:,column,0],'o-',alpha=1)
...
print(np.median(diff_axis1), np.median(diff_axis2))
...
pl.clim(0, 1)
# pl.figure(figsize=(5,10))
# pl.imshow(submask.T[::4,::4], cmap='gray')
# pl.colorbar(shrink=0.5)
# pl.clim(0, 1)
# upsampled_mask = np.stack([upsample_opencv(m, chunk_size, chunk_size) for m in mask_[:30]], axis=0)
#%%
...
#%%
bad_color = unique_colors_rows[locs[0]]
np.argwhere(layers[list(layers.keys())[1]]['face_color']==bad_color)
...
# locs_rows = np.argwhere(reverse_indices_columns==locs[1])
# print(locs_columns, locs_rows)
